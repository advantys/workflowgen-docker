...
$enableLocalhostHttps                    = (Get-WFGEnvVar "WFGEN_ENABLE_LOCALHOST_HTTPS" -DefaultValue "N" -TryAzureAppServices).ToUpper()
...
$setIISNodeSettingsFromEnv = { param ($Prefix)
    [Environment]::GetEnvironmentVariables().GetEnumerator() `
        ...
        | ForEach-Object {
            $configKey = $_.Key.Remove(0, $Prefix.Length)
            $separatorIndex = $configKey.IndexOf("_")

            if ($separatorIndex -lt 0 -or $separatorIndex -eq ($configKey.Length - 1)) {
                Write-WFGError "CONFIG: The given IISNode configuration key is invalid."
                exit 1
            }

            $applicationName = $configKey.Substring(0, $separatorIndex)
            $attributeName = $configKey.Substring($separatorIndex + 1)
        ...
        | ForEach-Object {
            ...
            Set-WFGIISNodeSetting -Name $_.Attribute.Name -Value $_.Attribute.Value -Application $_.Application
            Write-Host "done" -ForegroundColor Green
        }
}
...
$ensureLocalhostHttpsBinding = { param([string]$SiteName)
    $certificateFriendlyName = "WorkflowGen Docker Localhost"
    $certificateSubject = "CN=localhost"
    $sslBindingPath = "IIS:\SslBindings\0.0.0.0!443"

    Write-Host "CONFIG: Ensuring HTTPS localhost binding for site ""$SiteName"" ... " -NoNewline
    ...
    $httpsBinding = Get-WebBinding -Name $SiteName -Protocol "https" -ErrorAction SilentlyContinue `
        | Where-Object { $_.bindingInformation -eq "*:443:" }
    ...
    try {
        Get-Item ("Cert:\LocalMachine\My\{0}" -f $certificate.Thumbprint) `
...
$mcpConfigKeys = @(
    ...
    "OAuthServerUseDurableStore",
    "McpResourceUri",
    "McpAllowedScopes",
    "OAuthServerAccessTokenLifetimeSeconds",
    "OAuthServerAuthorizationCodeLifetimeSeconds",
    "OAuthServerRefreshTokenLifetimeSeconds",
    "McpDiagnosticLogLevel",
    "OAuthServerAllowLocalhostRedirects",
    "McpDevAuthBridgeMode",
    "McpDevAuthBridgeUsername"
)

$mcpConfigKeys | ForEach-Object {
    $value = Get-WFGEnvVar $_ -TryAzureAppServices

    if ([string]::IsNullOrEmpty($value)) {
        return
    }

    Write-Host "CONFIG: Setting MCP option ""$_"" ... " -NoNewline
    & $forceSetAppSettingsWebConfig $_ $value
    Write-Host "done" -ForegroundColor Green
}
...
switch -Regex ($authModeName.ToLower()) {
    $global:Constants.AUTH_MODE_APPLICATION {
        ...
        Set-IISAuthentication Anonymous -Location $global:Constants.IIS_SITE_LOCATION_MCP
        ...
        Set-IISAuthentication Windows -Location $global:Constants.IIS_SITE_LOCATION_MCP -Disable
        ...
        Set-IISAuthentication Basic -Location $global:Constants.IIS_SITE_LOCATION_MCP -Disable
...
if ($isWebServices -and $enableLocalhostHttps -eq "Y") {
    & $ensureLocalhostHttpsBinding -SiteName "wfgenroot"
}
